...
export class CategoriesService {
  ...
  // BUSCAR TODAS LAS CATEGORIAS ADMIN 
  // el admin puede ver las categorias borradas con soft-delete para llevar un mejor control
  async findAllCategoriesAdmin() {
    const catAdmin = await this.categoryRepository.find({
      relations: ['children'],
      withDeleted: true,
    });

    // diferenciar cuáles están borradas con soft delete y cuáles siguen activas
    return catAdmin.map((category) => ({ ...category, isDeleted: category.deletedAt !== null }));
  }
  ...
  async createCategory(categoryDTO: CreateCategoryDTO): Promise<CategoryEntity> {
    ...
    // revisar que el nombre de la categoria no exista en BD (incluye borradas con soft-delete)
    const existiCategory = await this.categoryRepository.findOne({ where: { slug }, withDeleted: true });
    if (existiCategory) {
  ...
  async removeCategory(id: string): Promise<{ message: string }> {
    ...
    await this.softDeleteTree(id); // llamda al metodo recursivo para eliminar las subcategorias
    return { message: 'Categoría eliminada correctamente' };
  }

  // METODO RECURSIVO PARA ELIMINAR CATEGORIAS Y TODOS SUS HIJOS
  private async softDeleteTree(id: string) {
  ...
  // RESTAURAR CATEGORIA Y SUS SUBCATEGORIAS
  async restoreCategory(id: string): Promise<{ message: string }> {
    ...
    await this.restoreTree(id); // metodo recursivo para restaurar las subcategorias que borro softDeleteTree()
    return { message: 'Categoría restaurada correctamente' };
  }

  // RESTAURAR la CATEGORIA y, RECURSIVAMENTE, todas sus SUBCATEGORIAS borradas
  private async restoreTree(id: string) {
    const children = await this.categoryRepository.find({ where: { parentId: id }, withDeleted: true });
    for (const child of children) {
      await this.restoreTree(child.id);
    }

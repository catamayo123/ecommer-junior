...
export class DownloadsService {
    ...
    for (const order of orders) {
      for (const item of order.items) {
        ...
        if (product.productType === ProductType.COURSE) {
          ...
          const isExpired = item.downloadTokenExpiresAt ? new Date() > item.downloadTokenExpiresAt : true;
  ...
  async payRenewal(paymentId: string, orderItemId: string, adminId: string) {
    ...
    item.downloadToken = this.jwtService.sign({ itemId: item.id, orderId: payment.orderId }, { expiresIn: '24h' });
